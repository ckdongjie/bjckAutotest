...
#         cmdStr = iperfPath+'\\iperf3 -c '+pdnIp+' -p '+port+' -i 1 -w 1000k -t 86400 -P 5 --logfile '+logFile
    if dir == 'DL':
        cmdStr = iperfPath+'\\iperf3 -s '+pdnIp+' -p '+port+'>>'+logFile
    elif dir == 'UL':
        cmdStr = iperfPath+'\\iperf3 -c '+pdnIp+' -p '+port+'>>'+logFile
    time.sleep(2)
...
#启动iperf服务端监听
def start_iperf_server(channel, dir, port, ip):
    if dir == 'DL':
        iperf_cmd = 'nohup iperf3 -c '+ip+' -p '+str(port)+' -i 1 -w 1000k -t 86400 -P 5 -R &'
    elif dir == 'UL':
        iperf_cmd = 'nohup iperf3 -c '+ip+' -p '+str(port)+' -i 1 -w 1000k -t 86400 -P 5 &'
...
def one_port_monitor(cpeIp, port, dir, testType):
    ...
    while IPERF_RUN == False:
        ...
        if PING_RUN == True:
            ...
            time.sleep(3)
            if testType == 'NR':   
                restart_iperf_server(port, dir, nrIp)
            elif testType == 'WIFI':
                restart_iperf_server(port, dir, wifiIp)
            time.sleep(3)
            IPERF_RUN = monitor_iperf(iperfPath, pdnIp, port, logFile, dir, errFile)
