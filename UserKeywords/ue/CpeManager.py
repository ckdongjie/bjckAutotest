...
def key_cpe_attach_cell_info(cpe):
    with allure.step(key_get_time() +": 查询cpe驻留状态及小区信息\n"):
        ...
        cellPci = CpeService().query_resident_cell_pci(cpe)
        with allure.step(key_get_time()+': ['+cpe.ip+'] 驻留小区PCI: '+str(cellPci)+'\n'):
            logging.info(key_get_time()+': ['+cpe.ip+'] resident cell pci: '+str(cellPci))
        return cellPci
...
'''      
def key_cpe_ping(cpe, pdnIp=BASIC_DATA['pdn']['pdnIp'], pingNum=BASIC_DATA['ping']['pingNum'], pingInterface = BASIC_DATA['cpe']['pingNrInterface'], 
            log_save_path=BASIC_DATA['ping']['logSavePath'], tryNum =5, pingSize=BASIC_DATA['ping']['pingSize'], 
            exptLossRate = BASIC_DATA['ping']['loseRate'],exptPingAvg = BASIC_DATA['ping']['pingAvg'], isCheckPing=BASIC_DATA['ping']['isCheckPing']):
    with allure.step(key_get_time() +": 执行ping包命令, 端口："+pingInterface):
        ...
        if isCheckPing == True:
            ...
            assert int(lossrate) <= exptLossRate, '端口:'+pingInterface+' ping包丢包率大于预期，请检查！'
            assert float(avg) <= exptPingAvg, '端口:'+pingInterface+' ping包平均时延大于预期，请检查！'
...
'''
    ...
    log_save_path:ping包log记得路径
'''      
def key_local_pc_ping(cpe, pdnIp=BASIC_DATA['pdn']['pdnIp'], pingNum=BASIC_DATA['ping']['pingNum'], tryNum=1, pingSize=BASIC_DATA['ping']['pingSize'], 
            exptLossRate = BASIC_DATA['ping']['loseRate'],exptPingAvg = BASIC_DATA['ping']['pingAvg'], isCheckPing=BASIC_DATA['ping']['isCheckPing']):
    with allure.step(key_get_time() +": 本地pc执行ping包命令"):
        logging.info(key_get_time()+': local pc exec ping command')
        for i in range (1, tryNum+1):
            min, avg, max, transmitted, received, lossrate = CpeService().local_pc_ping(pdnIp, pingNum, pingSize)
            if avg != -1:
                break
        with allure.step(key_get_time()+': Cpe['+cpe.ip+'] local PC ping result[max/avg/min/transmitted/received/loss rate] = '+str(max)+"/"+str(avg)+"/"+str(min)+"/"+str(transmitted)+"/"+str(received)+"/"+str(lossrate)+"%\n"):
            logging.info(key_get_time()+': Cpe['+cpe.ip+'] local PC  ping result[max/avg/min/transmitted/received/loss rate] = '+str(max)+"/"+str(avg)+"/"+str(min)+"/"+str(transmitted)+"/"+str(received)+"/"+str(lossrate)+'%')
        assert avg != -1,'ping包测试不通过，请检查'
        if isCheckPing == True:
...
def key_reboot_cpe(cpe):
    with allure.step(key_get_time()+': 复位cpe\n'):
        ...
        with allure.step(key_get_time()+':'+cpe.ip+' 复位成功'):
            logging.info(key_get_time()+':'+cpe.ip+' reboot success')
    ...
    with allure.step(key_get_time()+': 确认pdu创建成功\n'):
        ...
        if setupRes == 'success':
            with allure.step(key_get_time()+': cpe接入成功'):
                logging.info('cpe access success!')
        else:
            with allure.step(key_get_time()+': cpe接入失败'):
                logging.warning('cpe access failure!')
        return setupRes
    ...
    with allure.step(key_get_time()+': 确认pdu创建失败\n'):
        ...
        for i in range (1, tryNum):
            ...
            if setupRes == 'failure':
                ...
                key_wait(5)
        if setupRes == 'failure':
            with allure.step(key_get_time()+': cpe pdu释放成功'):
                logging.info('pdu release success!')
        else:
            with allure.step(key_get_time()+': cpe pdu释放失败'):
                logging.warning('pdu release failure!')
        return setupRes
        return setupRes  
...
def key_cpe_login_at_model(cpe):
    with allure.step(key_get_time() +": 登录cpe AT模式\n"):
        logging.info(key_get_time()+': '+cpe.ip+' login cpe AT model')
...
def key_cpe_logout_at_model(cpe):
    with allure.step(key_get_time() +": 登出cpe AT模式\n"):
        logging.info(key_get_time()+': '+cpe.ip+' logout cep AT model')
...
'''
    ...
    processNum:进程个数  
'''            
def key_dl_udp_wifi_flow_test(cpe, pdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['udpDlSize'], 
            monitorPort=BASIC_DATA['traffic']['wifiDlPort'], pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], 
            processNum=BASIC_DATA['traffic']['processNum'], spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'],
            isLocalExec=BASIC_DATA['traffic']['isLocalExec']):
    with allure.step(key_get_time() +": 下行UDP流量测试"):
        ...
        with allure.step(key_get_time() +": 执行iperf命令"):
            ...
            CpeService().cpe_udp_flow_DL(cpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            ...
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(cpe, enbDebugIp, pcIp, scrapFileName, dir = 'DL', pcNetworkCardName =pcNetworkCardName, spanTime=spanTime, type='WIFI', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(pdn, monitorPort)
        with allure.step(key_get_time() +": 流量测试结果：\n"+dlTrafRes):
            logging.info(key_get_time()+': '+cpe.ip+' DL UDP Wifi test result:\n'+dlTrafRes)
        return avgDlTraf
...
'''
    ...
    processNum:进程个数  
'''            
def key_dl_udp_nr_flow_test(cpe, pdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], 
            packageSize=BASIC_DATA['traffic']['udpDlSize'], monitorPort=BASIC_DATA['traffic']['nrDlPort'], 
            processNum=BASIC_DATA['traffic']['processNum'], spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'],
            isLocalExec=BASIC_DATA['traffic']['isLocalExec']):
    with allure.step(key_get_time() +": 下行UDP流量测试"):
        logging.info(key_get_time()+': exec DL UDP traffic test')
        ...
        with allure.step(key_get_time() +": 执行iperf命令"):
            ...
            CpeService().cpe_udp_flow_DL(cpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            logging.info(key_get_time()+': start scrap process, analyze data, capture time:'+str(spanTime))
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(cpe, enbDebugIp, pcIp, scrapFileName, dir = 'DL', pcNetworkCardName=pcNetworkCardName, spanTime=spanTime, type='NR', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(pdn, monitorPort)
        with allure.step(key_get_time() +": "+cpe.ip+" 流量测试结果：\n"+dlTrafRes):
            logging.info(key_get_time()+': '+cpe.ip+' DL UDP NR test result:\n'+dlTrafRes)
        return avgDlTraf
'''
        上行udp wifi灌包
        参数： 
    ...
    processNum:进程个数  
'''       
def key_ul_udp_wifi_flow_test(cpe, pdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['udpUlSize'],
            monitorPort=BASIC_DATA['traffic']['wifiUlPort'], pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], 
            processNum =BASIC_DATA['traffic']['processNum'], spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'],
            isLocalExec=BASIC_DATA['traffic']['isLocalExec']):
    with allure.step(key_get_time() +": 上行UDP流量测试"):
        ...
        with allure.step(key_get_time() +": 执行iperf命令"):
            logging.info(key_get_time()+': exec iperf command')
            CpeService().cpe_udp_flow_UL(cpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            logging.info(key_get_time()+': start scrap process, analyze data, capture time(s):'+str(spanTime))
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(cpe, enbDebugIp, pcIp, scrapFileName, dir = 'UL', pcNetworkCardName =pcNetworkCardName, spanTime=spanTime, type='WIFI', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(pdn, monitorPort)
        with allure.step(key_get_time() +": "+cpe.ip+" 流量测试结果：\n"+ulTrafRes):
            logging.info(key_get_time()+': '+cpe.ip+' UL UDP Wifi test result:\n'+ulTrafRes)
        return avgUlTraf
...
'''
    ...
    processNum:进程个数  
'''       
def key_ul_udp_nr_flow_test(cpe, pdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['udpUlSize'],
            monitorPort=BASIC_DATA['traffic']['nrUlPort'], pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], 
            processNum =BASIC_DATA['traffic']['processNum'], spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'],
            isLocalExec=BASIC_DATA['traffic']['isLocalExec']):
    with allure.step(key_get_time() +": 上行UDP流量测试"):
        logging.info(key_get_time()+': exec UL UDP traffic test')
        ...
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            logging.info(key_get_time()+': start scrap process, analyze data, capture time(s):'+str(spanTime))
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(cpe, enbDebugIp, pcIp, scrapFileName, dir = 'UL', pcNetworkCardName =pcNetworkCardName, spanTime=spanTime, type='NR', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(pdn, monitorPort)
        with allure.step(key_get_time() +": "+cpe.ip+" 流量测试结果：\n"+ulTrafRes):
            logging.info(key_get_time()+': '+cpe.ip+' UL UDP NR test result:\n'+ulTrafRes)
        return avgUlTraf
'''
        下行tcp wifi灌包
        参数：
    ...
    processNum:进程个数  
'''       
def key_dl_tcp_wifi_flow_test(cpe, pdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['tcpDlSize'],
            monitorPort=BASIC_DATA['traffic']['wifiDlPort'], pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], 
            processNum=BASIC_DATA['traffic']['processNum'], spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'],
            isLocalExec=BASIC_DATA['traffic']['isLocalExec']):
    with allure.step(key_get_time() +": 下行TCP流量测试"):
        ...
        with allure.step(key_get_time() +": 执行iperf命令"):
            logging.info(key_get_time()+': exec iperf command')
            CpeService().cpe_tcp_flow_DL(cpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            logging.info(key_get_time()+': start scrap process, analyze data, capture time(s):'+str(spanTime))
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(cpe, enbDebugIp, pcIp, scrapFileName, dir = 'DL', pcNetworkCardName =pcNetworkCardName, spanTime=spanTime, type='WIFI', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(pdn, monitorPort)
        with allure.step(key_get_time() +": "+cpe.ip+" 流量测试结果：\n"+dlTrafRes):
            logging.info(key_get_time()+': '+cpe.ip+' DL TCP Wifi test result:\n'+dlTrafRes)
        return avgDlTraf
'''
        下行tcp nr灌包
        参数：
    ...
    processNum:进程个数  
'''       
def key_dl_tcp_nr_flow_test(cpe, pdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['tcpDlSize'],
            monitorPort=BASIC_DATA['traffic']['nrDlPort'], pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], 
            processNum=BASIC_DATA['traffic']['processNum'], spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'],
            isLocalExec=BASIC_DATA['traffic']['isLocalExec']):
    with allure.step(key_get_time() +": 下行TCP流量测试"):
        ...
        key_stop_listen_port(pdn, monitorPort)
        with allure.step(key_get_time() +": "+cpe.ip+" 流量测试结果：\n"+dlTrafRes):
            logging.info(key_get_time()+': '+cpe.ip+' DL TCP NR test result:\n'+dlTrafRes)
        return avgDlTraf
'''
        上行tcp wifi灌包
        参数： 
    ...
    processNum:进程个数  
'''        
def key_ul_tcp_wifi_flow_test(cpe, pdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['tcpUlSize'],
            monitorPort=BASIC_DATA['traffic']['wifiUlPort'], pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], 
            processNum=BASIC_DATA['traffic']['processNum'], spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'],
            isLocalExec=BASIC_DATA['traffic']['isLocalExec']):
    with allure.step(key_get_time() +": 上行TCP流量测试"):
        ...
        with allure.step(key_get_time() +": 执行iperf命令"):
            logging.info(key_get_time()+': exec iperf command')
            CpeService().cpe_tcp_flow_UL(cpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            logging.info(key_get_time()+': start scrap process, analyze data, capture time(s):'+str(spanTime))
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(cpe, enbDebugIp, pcIp, scrapFileName, dir = 'UL', pcNetworkCardName =pcNetworkCardName,spanTime=spanTime, type='WIFI', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(pdn, monitorPort)
        with allure.step(key_get_time() +": "+cpe.ip+" 流量测试结果：\n"+ulTrafRes):
            logging.info(key_get_time()+': '+cpe.ip+' UL TCP Wifi test result:\n'+ulTrafRes)
        return avgUlTraf
'''
        上行tcp nr灌包
        参数： 
    ...
    processNum:进程个数  
'''        
def key_ul_tcp_nr_flow_test(cpe, pdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['tcpUlSize'],
            monitorPort=BASIC_DATA['traffic']['nrUlPort'], pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], 
            processNum=BASIC_DATA['traffic']['processNum'], spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'],
    ...
    with allure.step(key_get_time() +": 上行TCP流量测试"):
        ...
        key_stop_listen_port(pdn, monitorPort)
        with allure.step(key_get_time() +": "+cpe.ip+" 流量测试结果：\n"+ulTrafRes):
            logging.info(key_get_time()+': '+cpe.ip+' UL TCP NR test result:\n'+ulTrafRes)
        return avgUlTraf
...
'''
    ...
    processNum:进程个数  
'''        
def key_udl_tcp_nr_flow_test(ulCpe, dlCpe, ulPdn, dlPdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['tcpUlSize'],
            monitorUlPort=BASIC_DATA['traffic']['nrUlPort'], monitorDlPort=BASIC_DATA['traffic']['nrDlPort'], 
            pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], processNum=BASIC_DATA['traffic']['processNum'], 
            spanTime=BASIC_DATA['traffic']['spanTime'], gnbType=BASIC_DATA['common']['gnbType'], isLocalExec=BASIC_DATA['traffic']['isLocalExec']):
    with allure.step(key_get_time() +": 上下行TCP流量测试"):
        ...
        with allure.step(key_get_time() +": 执行iperf命令"):
            logging.info(key_get_time()+': exec iperf command')
            CpeService().cpe_tcp_flow_UL(ulCpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorUlPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
            CpeService().cpe_tcp_flow_DL(dlCpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorDlPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            logging.info(key_get_time()+': start scrap process, analyze data, capture time(s):'+str(spanTime))
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(ulCpe, enbDebugIp, pcIp, scrapFileName, dir = 'UDL', pcNetworkCardName =pcNetworkCardName,spanTime=spanTime, type='NR', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(ulPdn, monitorUlPort)
        key_stop_listen_port(dlPdn, monitorDlPort)
        with allure.step(key_get_time() +": "+ulCpe.ip+" 上行流量测试结果：\n"+ulTrafRes):
            logging.info(key_get_time()+': '+ulCpe.ip+' UL TCP NR test result:\n'+ulTrafRes)
        with allure.step(key_get_time() +": "+dlCpe.ip+" 下行流量测试结果：\n"+dlTrafRes):
            logging.info(key_get_time()+': '+dlCpe.ip+' DL TCP NR test result:\n'+dlTrafRes)
...
'''
    ...
    processNum:进程个数  
'''        
def key_udl_tcp_wifi_flow_test(ulCpe, dlCpe, ulPdn, dlPdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['tcpUlSize'],
            monitorUlPort=BASIC_DATA['traffic']['nrUlPort'], monitorDlPort=BASIC_DATA['traffic']['nrDlPort'], 
            pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], processNum=BASIC_DATA['traffic']['processNum'], 
    ...
    with allure.step(key_get_time() +": 上下行TCP流量测试"):
        ...
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            logging.info(key_get_time()+': start scrap process, analyze data, capture time(s):'+str(spanTime))
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(ulCpe, enbDebugIp, pcIp, scrapFileName, dir = 'UDL', pcNetworkCardName =pcNetworkCardName,spanTime=spanTime, type='WIFI', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(ulPdn, monitorUlPort)
        key_stop_listen_port(dlPdn, monitorDlPort)
        with allure.step(key_get_time() +": "+ulCpe.ip+"上行流量测试结果：\n"+ulTrafRes):
            logging.info(key_get_time()+': '+ulCpe.ip+' UL TCP WIFI test result:\n'+ulTrafRes)
        with allure.step(key_get_time() +": "+dlCpe.ip+"下行流量测试结果：\n"+dlTrafRes):
            logging.info(key_get_time()+': '+dlCpe.ip+' DL TCP WIFI test result:\n'+dlTrafRes)
        return avgDlTraf, avgUlTraf
'''
        上下行udp nr灌包
        参数： 
    ...
    processNum:进程个数  
'''        
def key_udl_udp_nr_flow_test(ulCpe, dlCpe, ulPdn, dlPdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['tcpUlSize'],
            monitorUlPort=BASIC_DATA['traffic']['nrUlPort'], monitorDlPort=BASIC_DATA['traffic']['nrDlPort'], 
            pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], processNum=BASIC_DATA['traffic']['processNum'], 
    ...
    with allure.step(key_get_time() +": 上下行UDP流量测试"):
        ...
        with allure.step(key_get_time() +": 执行iperf命令"):
            logging.info(key_get_time()+': exec iperf command')
            CpeService().cpe_udp_flow_UL(ulCpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorUlPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
            CpeService().cpe_udp_flow_DL(dlCpe, cpePcIp, iperfPath, pdnIp, packageSize, monitorDlPort, processNum, spanTime=spanTime, isLocalExec=isLocalExec)
        with allure.step(key_get_time() +": 启动抓包程序，进行数据分析"):
            logging.info(key_get_time()+': start scrap process, analyze data, capture time(s):'+str(spanTime))
            dlTrafRes,ulTrafRes, avgDlTraf, avgUlTraf = CpeService().cell_flow_analyze(ulCpe, enbDebugIp, pcIp, scrapFileName, dir = 'UDL', pcNetworkCardName =pcNetworkCardName,spanTime=spanTime, type='NR', gnbType=gnbType, isLocalExec=isLocalExec)
        key_stop_listen_port(ulPdn, monitorUlPort)
...
'''
    ...
    processNum:进程个数  
'''        
def key_udl_udp_wifi_flow_test(ulCpe, dlCpe, ulPdn, dlPdn, cpePcIp=BASIC_DATA['traffic']['cpePcIp'], iperfPath=BASIC_DATA['traffic']['iperfLocalPath'], 
            pdnIp=BASIC_DATA['pdn']['pdnIp'], enbDebugIp=BASIC_DATA['weblmt']['ip'], pcIp=BASIC_DATA['traffic']['localPcIp'], 
            scrapFileName=BASIC_DATA['traffic']['scrapFileName'], packageSize=BASIC_DATA['traffic']['tcpUlSize'],
            monitorUlPort=BASIC_DATA['traffic']['nrUlPort'], monitorDlPort=BASIC_DATA['traffic']['nrDlPort'], 
            pcNetworkCardName=BASIC_DATA['traffic']['pcNetworkCardName'], processNum=BASIC_DATA['traffic']['processNum'], 
...
'''
           功能：启动ue log跟踪
'''
def key_start_ue_log_trace(isSaveQxdm = BASIC_DATA['maintenanceTool']['isSaveQxdm']):
    if isSaveQxdm == True:
...
'''
           ...
           参数：
'''
def key_stop_ue_log_trace(dev_manager, qxdm_window, diagService, isSaveQxdm = BASIC_DATA['maintenanceTool']['isSaveQxdm']):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    logSavePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
    if isSaveQxdm == True:
...
'''
           功能：停止ue log跟踪并对文件进行另存
           参数：
'''
def key_qxdm_log_save(ueLogFilePath='', isSaveQxdm = BASIC_DATA['maintenanceTool']['isSaveQxdm']):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogBackup = BASE_DIR+'\\AutoTestMain\\qxdmLog\\backup'
    if isSaveQxdm == True:
        with allure.step(key_get_time()+': 保存qxdm log到备份目录'):
            logging.info(key_get_time()+': move the log file to backup folder')
            shutil.move(ueLogFilePath, ueLogBackup+'\\'+time.strftime("%Y%m%d%H%M%S", time.localtime())+'.hdf')
...
'''
        ...
        symbolNumber：时隙配置参数
'''    
def key_pdcch_symbol_number_analyze(symbolNumber='Adaptive'):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        cceLevel：cce聚合等级
'''     
def key_pdcch_cce_level_analyze(cceLevel):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        transferFormat：传输格式
'''     
def key_pdcch_transfer_formate_analyze(transferFormat):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        index：索引值
'''     
def key_prach_config_index_analyze(index):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''     
def key_pucch_support_format_analyze(formatType):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        logFilePath：ue log文件路径
'''     
def key_pucch_support_format1_format3_hop_analyze():
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        logFilePath：ue log文件路径
'''     
def key_support_dmrs_mapping_type_a():
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        logFilePath：ue log文件路径
'''     
def key_support_csi_rs(trsPeriod):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        logFilePath：ue log文件路径
'''     
def key_sib2_smtc_analyze(smtcPeriod):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        logFilePath：ue log文件路径
'''     
def key_sib1_ssb_period_analyze(ssbPeriod):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        logFilePath：ue log文件路径
'''     
def key_ssb_frequenty_position_analyze(ssbFreqPos):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''     
def key_pre_rx_power_and_power_ramp_analyze(preambleReceivedTargetPower, powerRampingStep):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        logFilePath：ue log文件路径
'''     
def key_po_nominal_pusch_analyze(poNominal):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        logFilePath：ue log文件路径
'''     
def key_path_loss_coefficient_analyze(alpha):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        powerRampingStep：功率调整步长
'''     
def key_system_info_parameter_analyze(subcarrierSpacing, nrofDownlinkSlots, nrofDownlinkSymbols):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
    with allure.step(key_get_time()+':校验功率初始值和功率调整步长参数是否生效'):
        logging.info(key_get_time()+':check pre rx power and power ramp')
...
'''     
def key_csi_rs_power_analyze(csiRsPowerOffset):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''     
def key_pusch_resource_allocation_type_analyze(puschAllocType):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''     
def key_pdsch_resource_allocation_type_analyze(pdschAllocType):
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
...
'''
        ...
        pdschAllocType：pdsch资源分配类型
'''     
def key_pucch_channel_feedback_per_bandwith_analyze():
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) #d:/bjckAutotest
    ueLogFilePath = BASE_DIR+'\\AutoTestMain\\qxdmLog'
    with allure.step(key_get_time()+':校验pdsch资源分配类型参数是否生效'):
        logging.info(key_get_time()+':check pdsch resource allocation type')
