...
def testUpdateOmcIpScence1(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], hmsIp=BASIC_DATA['hms']['ip'], hmsIp2=BASIC_DATA['hms']['ip2'], oldOmcIp=BASIC_DATA['hms']['omcIp'], newOmcIp=BASIC_DATA['hms']['omcIp2']):
    with allure.step('更新omc ip地址测试：修改地址正确且基站在新网管已经注册'):
        souHmsObj=key_login_hms(hmsIp)
        souEnbId, souEnbName = key_get_enb_info(souHmsObj, serialNumber)
        desHmsObj=key_login_hms(hmsIp2)
        desEnbId, desEnbName = key_get_enb_info(desHmsObj, serialNumber)
        for i in range (1,testNum+1):
            logging.info(key_get_time()+':run the test <'+str(i)+'> times')
            with allure.step(key_get_time()+':执行第 '+str(i)+'次测试'):
                key_confirm_device_online(souHmsObj, serialNumber)
                key_modify_omc_url(souHmsObj, souEnbId, newOmcIp)
                with allure.step(key_get_time()+': omc ip修改生效，等待10s检查基站状态'):
                    key_wait(60)
                key_confirm_device_status_same_as_expect(souHmsObj, serialNumber,'offline')
                #登录另一个omc系统，确认基站在线
                key_confirm_device_online(desHmsObj, serialNumber)
                with allure.step(key_get_time()+': 恢复基站参数'):
                    key_modify_omc_url(desHmsObj, desEnbId, oldOmcIp)
                with allure.step(key_get_time()+': omc ip修改生效，等待10s检查基站状态'):
                    key_wait(60)
                key_confirm_device_status_same_as_expect(desHmsObj, serialNumber,'offline')
...
def testUpdateOmcIpScence2(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], hmsIp=BASIC_DATA['hms']['ip'], oldOmcIp=BASIC_DATA['hms']['omcIp'], newOmcIp=BASIC_DATA['hms']['omcIp2']):
...
def testUpdateOmcIpScence3(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], hmsIp=BASIC_DATA['hms']['ip'], oldOmcIp=BASIC_DATA['hms']['omcIp'], newOmcIp=BASIC_DATA['hms']['erroromcIp']):
    with allure.step('更新omc ip地址测试：修改地址正确但基站在新网管未注册'):
        souHmsObj=key_login_hms(hmsIp)
        souEnbId, souEnbName = key_get_enb_info(souHmsObj, serialNumber)
        for i in range (1,testNum+1):
...
def testSctpRemotePortConfig():
    with allure.step('SCTP目标端口号修改测试'):
        ...
        key_check_active_alarm_with_white_list(hmsObj, serialNumber, alarmWhiteList)
        #确认小区状态不正常
        ...
        key_check_active_alarm_with_black_list(hmsObj, serialNumber, alarmBlackList)
        #确认小区状态不正常
        key_confirm_cell_status(hmsObj, enbId, 'available')