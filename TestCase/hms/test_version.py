            ...
            if attachRes == 'OK':
                ...
                if isFlow:
                    with allure.step(key_get_time()+': 小区流量测试'):
                        logging.info(key_get_time()+': start cell flow test.')
...
#         启动基站抓包
    ...
    downRes = key_download_version(hmsObj, serialNumberList, softVersion)
    assert downRes == 'success', '版本下载执行失败，请检查！'
    downStatus = key_query_download_status(hmsObj, enbName)
    assert downStatus == 'success','基站版本下载失败，请检查！'
    key_query_version_info_from_device(hmsObj, serialNumberList)
    ...
    with allure.step(key_get_time()+':确认版本升级成功，校验升级后版本号是否正确'):
        ...
        if isCheckVerDetail == True:
            checkUpgradeVersionInfo(hmsObj, serialNumberList)
    if isCheckCell:
        key_confirm_cell_status(hmsObj, enbId, 'available')
    key_wait(60)
    #业务测试
    ...
    rollRes = key_rollback_version(hmsObj, serialNumberList)
    ...
    if rollStatus == 'rollbacking':
        ...
        with allure.step(key_get_time()+':等待基站回退复位启动'):
            ...
            if isCheckRollVerDetail == True:
                checkRollbackVersionInfo(hmsObj, serialNumberList)
    if isDownAgain == True: 
        with allure.step(key_get_time()+':下载中间版本，恢复环境'):
            logging.warning(key_get_time()+': download other version')
            downRes = key_download_version(hmsObj, serialNumberList, recoverVersion)
            assert downRes == 'success', '版本下载执行失败，请检查！'
            downStatus = key_query_download_status(hmsObj, enbName)
            assert downStatus == 'success','基站版本下载失败，请检查！'
            key_query_version_info_from_device(hmsObj, serialNumberList)
            key_query_version_info(hmsObj, serialNumberList)
    if isRollCpld == True:
        #手工更新cpld版本
        gnb = key_ssh_login_gnb(gnbSshIp, gnbSshUser, gnbSshPass)
    ...
    if isXmlTest==True:
        exportAndImportXml(hmsObj, serialNumberList)

'''
                配置文件导出导入测试
'''
def exportAndImportXml(hmsObj, serialNumber):
    enbId, enbName = key_get_enb_info(hmsObj, serialNumber)
    xmlSavePath = BASIC_DATA['version']['xmlSavePath']
    checkData=BASIC_DATA['version']['isCheckData']
...
'''
                检查升级后小版本信息是否正确
'''
def checkUpgradeVersionInfo(hmsObj, serialNumberList):
...
'''
                检查升级后小版本信息是否正确
'''
def checkRollbackVersionInfo(hmsObj, serialNumberList):
...
def testGnbOfflineDuringDownloadVersion(testNum, waitTime, serialNumber=BASIC_DATA['gnb']['serialNumberList'], omcIp=BASIC_DATA['hms']['omcIp'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
        ...
        try:
            for i in range (1,testNum+1):
                ...
                with allure.step(key_get_time()+'执行第 '+str(i)+'次测试'):
                    key_upload_version_to_hms_if_version_no_exit(hmsObj, softVersion, savePath)
                    logging.info(key_get_time()+': download version to enb')
                    downloadRes = key_download_version(hmsObj, serialNumber, softVersion)
                    ...
                    with allure.step(key_get_time()+':基站版本恢复'):
                        ...
                        downloadRes = key_download_version(hmsObj, serialNumber, recoverVersion)
                        assert downloadRes == 'success','版本下载执行失败，请检查！'
                        downStatus = key_query_download_status(hmsObj, enbName)
                        assert downStatus == 'success','基站版本下载失败，请检查！'
                        key_query_version_info_from_device(hmsObj, serialNumber)
                        key_query_version_info(hmsObj, serialNumber)
            abnormal = False
        finally:
            if abnormal == True:
...
def testRepeatDownloadVersion(testNum, serialNumberList=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], softVersion2=BASIC_DATA['version']['upgradeVersion2'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
    with allure.step('异常测试：基站版本下载过程中再次执行版本下载'):
        ...
        try:
            for i in range (1,testNum+1):
                ...
                with allure.step(key_get_time()+':执行第 '+str(i)+'次测试'):
                    key_upload_version_to_hms_if_version_no_exit(hmsObj, softVersion, savePath)
                    key_upload_version_to_hms_if_version_no_exit(hmsObj, softVersion2, savePath)
                    downloadRes = key_download_version(hmsObj, serialNumberList, softVersion)
                    assert downloadRes == 'success','版本下载执行失败，请检查！'
                    #再次执行版本下载
                    reDownloadRes = key_download_version(hmsObj, serialNumberList, softVersion2)
                    ...
                    with allure.step(key_get_time()+':基站版本恢复'):
                        logging.info(key_get_time()+': recover gnb version')
                        downloadRes = key_download_version(hmsObj, serialNumberList, recoverVersion)
                        assert downloadRes == 'success','版本下载执行失败，请检查！'
                        downStatus = key_query_download_status(hmsObj, enbName)
                        assert downStatus == 'success','基站版本下载失败，请检查！'
                        key_query_version_info_from_device(hmsObj, serialNumberList)
...
def testActiveDuringDownloadVersion(testNum, serialNumberList=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
...
def testRollbackDuringDownloadVersion(testNum, serialNumberList=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
        ...
        try:
            for i in range (1,testNum+1):
                ...
                with allure.step(key_get_time()+':执行第 '+str(i)+'次测试'):
                    ...
                    #执行版本回退
                    key_wait(2)
                    activeRes = key_rollback_version(hmsObj, serialNumberList)
...
def testOmcRebootDuringDownloadVersion(testNum, serialNumberList=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
...
def testGnbOfflineDuringActiveVersion(testNum, waitTime, errorOmcIp, serialNumber=BASIC_DATA['gnb']['serialNumberList'], omcIp=BASIC_DATA['hms']['omcIp'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
        ...
        try:
            for i in range (1,testNum+1):
                ...
                with allure.step(key_get_time()+':执行第 '+str(i)+'次测试'):
                    ...
                    assert downStatus == 'success','基站版本下载失败，请检查！'
                    #激活版本包
                    activeRes = key_active_version(hmsObj, serialNumber)
                    ...
                    with allure.step(key_get_time()+':基站版本恢复'):
                        logging.info(key_get_time()+': recover gnb version')
                        rollRes = key_rollback_version(hmsObj, serialNumber)
                        assert rollRes == 'success','版本回退执行失败，请检查！'
                        rollStatus = key_query_rollback_status(hmsObj, enbName)
                        assert rollStatus == 'success','回退任务失败，请检查！'
                        with allure.step(key_get_time()+': 等待基站回退复位启动'):
...
def testDownloadDuringActiveVersion(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
...
def testActiveDuringActiveVersion(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
...
def testRollbackDuringActiveVersion(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
...
def testDownloadDuringRollbackVersion(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
...
def testRollbackDuringRollbackVersion(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
    with allure.step('异常测试：基站版本包回退过程中下载版本'):
        abnormal = True
        hmsObj = key_login_hms(BASIC_DATA['hms']['ip'])
        enbId, enbName = key_get_enb_info(hmsObj, serialNumber)
...
#                         key_login_hms()
                    ...
                    rollbackRes = key_rollback_version(hmsObj, serialNumber)
...
def testModifyParasDuringDownloadVersion(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
...
def testModifyParamDuringActiveVersion(testNum, waitTime, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
    with allure.step('异常测试：基站版本包激活过程中修改参数'):
        ...
        try:
            for i in range (1,testNum+1):
                ...
                with allure.step(key_get_time()+':执行第 '+str(i)+'次测试'):
                    key_upload_version_to_hms_if_version_no_exit(hmsObj, softVersion, savePath)
                    key_upload_version_to_hms_if_version_no_exit(hmsObj, recoverVersion, savePath)
                    downloadRes = key_download_version(hmsObj, serialNumber, softVersion)
                    assert downloadRes == 'success','版本下载执行失败，请检查！'
                    downStatus = key_query_download_status(hmsObj, enbName)
                    assert downStatus == 'success','基站版本下载失败，请检查！'
...
def testModifyParamDuringRollbackVersion(testNum, waitTime, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
...
def testDiffUserVersionUpgradeAndRollback(testNum, serialNumber=BASIC_DATA['gnb']['serialNumberList'], softVersion=BASIC_DATA['version']['upgradeVersion'], recoverVersion=BASIC_DATA['version']['recoverVersion'], savePath=BASIC_DATA['version']['versionSavePath']):
        ...
        try:
            for i in range (1,testNum+1):
                ...
                with allure.step(key_get_time()+'执行第 '+str(i)+'次测试'):
                    ...
                    autoDownloadRes = key_download_version(hmsObj2, serialNumber, softVersion, user='auto')
